...
def read_sample(predict_sample: PredictSample) -> pd.DataFrame:
    ...
    # directly create DataFrames from the sensor data dicts
    accel_df = pd.DataFrame(predict_sample.accel_data.model_dump())
    accel_df["timestamp"] = pd.to_timedelta(accel_df.timestamp, unit="ms")
    gyro_df = pd.DataFrame(predict_sample.gyro_data.model_dump())
    gyro_df["timestamp"] = pd.to_timedelta(gyro_df.timestamp, unit="ms")

    # use the minimum timestamp as origin
...
def extract_features(eval_df: pd.DataFrame) -> pd.DataFrame:
    ...
    eval_df_length = pd.DataFrame({"length": [len(eval_df)]})
    eval_df_extra = pd.concat([eval_df_mean, eval_df_std, eval_df_length], axis=1)
